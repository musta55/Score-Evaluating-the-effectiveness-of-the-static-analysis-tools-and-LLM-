...
main_dir ="../Score- Benchmark/results"
bug_types = [
...
{'tool':'LLM','bugs':['Re-entrancy','Timestamp-Dependency','Unchecked-Send','Unhandled-Exceptions','TOD','Overflow-Underflow','tx.origin']}]
            ...
            for cs in x:
                ...
                if tool =='Securify':
                    ...
                    for cs_name in cs_names[0]:
                        result_file = injected_scs+"/results/buggy_"+str(cs)+".sol_"+cs_name+".json"
...
if __name__ == "__main__":
    if len(sys.argv) != 3 or sys.argv[1] != '--tool':
        print(f"Usage: {sys.argv[0]} --tool <tool_name>")
        print("Available tools: Oyente, Securify, Mythril, Smartcheck, Manticore, Slither, LLM")
        sys.exit(1)
    
    tools = [sys.argv[2]]  # Put the tool name in a list
